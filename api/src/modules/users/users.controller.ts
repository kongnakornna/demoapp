...
export class UsersController {
  ...
  async listUser(
    ...
    let active_status: string = query.active;
    var sort: string = query.sort || 'createddate-ASC';
    ...
    filter.isCount = 1;
    let rowResultData: any = await this.usersService.listpaginate(filter);
    ...
    filter2.isCount = 0;
    let ResultData: any = await this.usersService.listpaginate(filter2);
    let tempData = [];
    let tempDataoid = [];
    let tempData2 = [];
