...
export class AuthService {
  ...
  async generateToken(ids: any): Promise<string> {
    ...
    //console.info(ids)
    const tokenGen: any = this.jwtService.sign({ id: ids });
  ...
  async generateTokenTime(ids: any): Promise<string> {
    //console.log('generateToken ids =>')
    //console.info(ids)
  ...
  public async generateRefreshToken(
    ...
    const token = await this.jwtService.signAsync(
      { id: id },
      {
        expiresIn: expiresIn || process.env.EXPIRE_TOKEN,
        secret: process.env.SECRET_KEY,
      },
    );
    return token;
